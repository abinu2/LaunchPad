  ...
  const handleAnalyzePrice = async () => {
    ...
    try {
      const serviceCatalog = new Map(
        (business.serviceTypes ?? []).map((service) => [service.id, service])
      );
      ...
      const res = await fetch("/api/ai/generate-quote", {
        ...
        body: JSON.stringify({
          services: lineItems.map((item) => ({
            ...
            supplyCost: serviceCatalog.get(item.serviceId)?.supplyCost ?? 0,
          })),
          vehicleType: vehicleType.label,
          businessType: business.businessType,
          city: business.businessAddress.city,
          state: business.businessAddress.state || business.entityState,
        }),
      });
      ...
      setPricingAnalysis(await res.json());
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      await addQuote(business.id, {
        ...
        taxAmount,
        total,
        pricingAnalysis: pricingAnalysis ?? undefined,
        status: "draft",
        sentAt: null,
        viewedAt: null,
        acceptedAt: null,
                    ...
                    {VEHICLE_TYPES.map((vt) => (
                      <button
                        ...
                        onClick={() => {
                          ...
                          setPricingAnalysis(null);
                          setSelectedServices((prev) =>
                            prev.map((s) => ({ ...s, vehicleMultiplier: vt.multiplier }))
                          );
                        }}
            ...
            <>
              ...
              <div className="rounded-xl border border-slate-200 p-4 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <Button onClick={handleAnalyzePrice} loading={analyzingPrice} size="sm">
                    {analyzingPrice ? "Analyzing..." : pricingAnalysis ? "Re-run analysis" : "Analyze price"}
                  </Button>
                </div>

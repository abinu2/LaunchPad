        ...
        {deadlines.map((d, i) => (
          <Link key={i} href={`/contracts/${d.id}`} className="flex items-center justify-between group">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-white/70 group-hover:text-[#00CF31] transition-colors">
                {d.label}
              </span>
            </div>
            <span className={`text-xs font-medium ${d.days <= 14 ? "text-red-400" : "text-white/50"}`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-white/70 border border-white/15 glass-card rounded-lg hover:bg-white/5 transition-colors"
            >
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-2 text-sm text-white bg-[#00CF31] rounded-lg hover:bg-blue-700 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        {contracts.length > 0 && (
          <div className="flex items-center justify-between gap-3 flex-wrap">
            <div className="flex gap-1 glass-card border border-white/10 rounded-lg p-1">
            ...
            <select
              ...
              className="text-xs text-white/60 border border-white/10 rounded-lg px-2 py-1.5 bg-white/5"
            >
        ...
        {sorted.length === 0 && !showUpload ? (
          <div className="glass-card rounded-xl border border-dashed border-white/15 p-16 text-center">
            ...
            <p className="text-white/60 font-medium mb-1">No contracts yet</p>
            <p className="text-white/40 text-sm mb-5">Upload a contract to get AI-powered clause analysis, risk scoring, and obligation tracking.</p>
            <button
              onClick={() => setShowUpload(true)}
              className="px-4 py-2 bg-[#00CF31] text-black font-semibold hover:bg-[#00b82c] transition-colors"
            >
...
function ContractCard({ contract: c }: { contract: Contract }) {
  ...
  return (
    <Link
      href={`/contracts/${c.id}`}
      className="block glass-card p-4 hover:border-blue-300 hover:shadow-sm transition-all"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            <p className="font-semibold text-white truncate">{c.counterpartyName}</p>
            {c.healthScore !== undefined && (
              <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${
                c.healthScore >= 80 ? "bg-green-500/15 text-green-400" :
                c.healthScore >= 60 ? "bg-amber-500/15 text-amber-400" :
          ...
          {urgentDays !== null && urgentDays <= 30 && (
            <p className={`text-xs mt-1 font-medium ${urgentDays <= 7 ? "text-red-400" : "text-amber-400"}`}>
